...
# VUE : ParentStatsOverviewView

class ParentStatsOverviewView(APIView):
    """
    Vue pour les statistiques globales du tableau de bord parent.
    Retourne le nombre d'enfants liés au parent connecté.
    Les stats de score sont gérées par l'app mlt_quiz.
    """
    permission_classes = [permissions.IsAuthenticated]

    def get(self, request):

        # Nombre d'enfants liés à ce parent
        total_enfants = Enfant.objects.filter(
            suiviparentenfant__parent__utilisateur=request.user
        ).count()

        return Response({
            "totalEnfants":      total_enfants,
            "moyenneGenerale":   None,  # Calculé par mlt_quiz
            "exercicesTermines": None,  # Calculé par mlt_quiz
